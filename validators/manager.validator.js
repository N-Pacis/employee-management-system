...
exports.validateManager = async(req,res,next)=>{
    ...
    if (checkNationalID) return res.status(400).send({
    ...
    if((req.body.Phone).length < 10 || (req.body.Phone).length > 10) return res.status(400).send({
    ...
    if(req.body.DateOfBirth){
        ...
        if((today.getFullYear() - date) < 18){
            return res.status(400).send({
               message: "You are not eligible to register because the provided age is less than 18 years"
            })
        }
    }
    return next()
